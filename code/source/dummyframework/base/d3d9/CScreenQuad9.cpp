...
namespace DummyFramework
{
    ...
    //=============================================================================================================
    bool CDynamicQuadBuffer9::Initialize(LPDIRECT3DDEVICE9 gfx, unsigned long quadnum)
    {
        device = gfx;
        quads = quadnum;
		...
		//hr = device->CreateIndexBuffer(quads * 6 * sizeof(unsigned long), 0, D3DFMT_INDEX32, D3DPOOL_MANAGED, &indexbuffer, NULL);
    ...
    void CDynamicQuadBuffer9::WriteQuad(size_t index, const D3DXVECTOR2& position, const D3DXVECTOR2& size,
		float texcoords[4], unsigned long color, bool flipuv)
    {
	...
	void CDynamicQuadBuffer9::WriteQuad(size_t index, D3DXVECTOR2 positions[4], float texcoords[4],
		unsigned long color, bool flipuv)
	{
		dnerror(, "CDynamicQuadBuffer9::WriteQuad(): Buffer overflow", index >= quads);
        sscreenvertex* vertices = vdata + index * 4;
